...
def on_policy_training(data_reader,
            ...
            if args.disparity_type != 'ashudeep_mod':
                ...
                for data in indicator_dataloader:
                    ...
                    if not JK:
                        ...
                        p_mat = policy_lp(scores.repeat(1,1,scores.shape[1]).squeeze(), group_identities)
                        p_mat = relu( p_mat )
                        #policy_lp = PolicyLP(N=scores.shape[1], eps=1e-3, indices_A=None)
                        #p_mat = policy_lp(scores.repeat(1,1,scores.shape[1]).squeeze())
                        rankings = sample_double_stoch(
                            p_mat, sample_size, return_prob=False, batch=True)
    ...
    for epoch in epoch_iterator:
        ...
        for batch_id, data in enumerate(train_dataloader):
            ...
            #JK = True     #JK remove
            #if not JK:    #JK remove
            if not (args.DSM or args.DSMfair):
                ...
                if args.DSMfair:
                    ...
                    #policy_lp = PolicyLP(N=scores.shape[1], eps=1e-3, indices_A=None)
                    #p_mat = policy_lp(scores.repeat(1,1,scores.shape[1]).squeeze())
                    rankings, log_model_prob = sample_double_stoch(
                    ...
                    print("Error: Specify either DSM or DSMfair in order to use ")
...
def soft_policy_training(data_reader,
    ...
    for epoch in epoch_iterator:
        ...
        for batch_id, data in enumerate(train_dataloader):
            ...
            """
            print("feats.get_device() = ")
            print( feats.get_device() )
            print("rel.get_device() = ")
            print( rel.get_device() )
            print("group_identities.get_device() = ")
            print( group_identities.get_device()  )
            """
            policy_lp = PolicyLP_Plus(N=scores.shape[1], position_bias_vector = position_bias_vector)
            p_mat = policy_lp(score_cross, group_identities)
            if args.gpu:
                policy_lp = policy_lp.cuda()   # JK testing
            p_mat = relu( p_mat )
            print("Policy LP call complete")
            #policy_lp = PolicyLP(N=scores.shape[1], eps=1e-3, indices_A=None)
            #p_mat = policy_lp(scores.repeat(1,1,scores.shape[1]).squeeze())
            #rankings, log_model_prob = sample_double_stoch(
            #        p_mat, sample_size, return_prob=True, batch=True)
